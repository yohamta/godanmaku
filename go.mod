module github.com/yotahamada/godanmaku

go 1.14

require (
	github.com/golang/freetype v0.0.0-20170609003504-e2365dfdc4a0
	github.com/hajimehoshi/ebiten v1.12.0-alpha.10
	github.com/hajimehoshi/go-inovation v0.0.0-20200828134421-2b466f1a7a6f // indirect
	github.com/hajimehoshi/wasmserve v0.0.0-20200812063057-947821f8a49c // indirect
	github.com/kr/pretty v0.1.0 // indirect
	github.com/mdempsky/gocode v0.0.0-20200405233807-4acdcbdea79d // indirect
	github.com/nsf/gocode v0.0.0-20190302080247-5bee97b48836 // indirect
	golang.org/x/exp v0.0.0-20190731235908-ec7cb31e5a56
	golang.org/x/image v0.0.0-20200801110659-972c09e46d76
	golang.org/x/tools v0.0.0-20200814230902-9882f1d1823d // indirect
)
